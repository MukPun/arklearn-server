...
**Agent = PlayerObject（玩家对象）**：

```
┌─────────────────────────────────────────────────────────────┐
│  Agent (PlayerObject)                                      │
│                                                              │
│  ├── 对外接口（供其他 Skynet Service 查询）                 │
│  │     ├── GetPlayerData(uid) → PlayerData                 │
│  │     ├── GetFightPower(uid) → number                      │
│  │     └── Query接口通过 skynet 消息实现                   │
│  │                                                          │
│  ├── 内部实现                                               │
│  │     └── ECS World ─ 管理玩家数据                         │
│  │                                                          │
│  └── 数据变化上报                                           │
│        └── OnDataChanged → 通知其他 Service                 │
└─────────────────────────────────────────────────────────────┘
```
...
| 接口 | 调用方 | 说明 |
|------|--------|------|
| `GetPlayerData(uid)` | RankingService, TeamManager 等 | 获取玩家完整数据 |
| `GetFightPower(uid)` | RankingService | 获取战斗力（排行榜用） |
| `OnDataChanged(type, data)` | 内部回调 → 上报聚合服务 | 数据变化时主动上报 |
...
- RankingService 依赖 Agent 上报的战斗力数据，而非直接读 DB
- TeamManager 依赖 Agent 提供的玩家数据
- 数据流：Agent 变化 → 上报 → 聚合服务

**扩展性**：
- Agent 作为统一入口，ECS Processor 在 Agent 内运行
- 重型逻辑可独立成 Actor（通过消息通信）
...
**与 Skynet Service 的数据交互**：
- Agent（PlayerObject）持有 ECS World
- 其他 Skynet Service（如 RankingService）通过 Agent 的接口获取数据
- 数据变化时，Agent 上报给聚合 Service
...
│   │   ├── item_stack.lua        # 物品组件
│   │   └── ...                   # 更多组件按需添加
│   └── systems/               	  # 玩法逻辑 (S - 逻辑，在 Agent World 内运行)
│       ├── save_systems.lua      # 存盘逻辑
│       ├── load_systems.lua      # 加载逻辑
│       └── ...                   # 更多 systems 按需添加
│
